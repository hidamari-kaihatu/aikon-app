...
//まず、名前とメールのフォームを作りバリエーションをかける[OK]
// 名前：必須、全角[OK]
// メール：必須、メール形式[OK]
//パスワードのフォームを作り、バリデーションをかける[OK]
// パスワード：必須、半角英数、８文字以上、確認パスワードとの一致[OK]
// パスワード確認：必須、半角英数、８文字以上、確認パスワードとの一致[OK]
//パスワードの目隠し機能を追加する[OK] 
// ４つの項目がdataとして取れたら、それを引数にしてonSubmit関数にfirebaseにstaffEmail, passwordを渡し、新規登録[]
//登録しましたのポップアップ
//ログインページへの遷移[OK]
type Inputs = {
  name: string,
